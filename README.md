...
`grovie init` still writes an optional, documented repo-local `.grovie.yml` with safe local-runner policy defaults. When the file is absent, Grovie uses the same built-in defaults. This file is policy configuration, not repository identity or daemon scheduling. `grovie doctor` validates the global worker config and any `.grovie.yml` in the current directory, then confirms the current `gh` login plus Codex CLI availability. Grovie stores local runner state under `~/.grovie/`.
