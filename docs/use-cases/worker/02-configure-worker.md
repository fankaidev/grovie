...
| UC-WORKER-02-S04 | P0 | A `watchedRepositories` entry can set safe repository policy for that repository's daemon runs without changing the daemon's machine id. |
| UC-WORKER-02-S05 | P1 | Global state repo config stores repo, branch, and sync interval for optional remote state sync. |
