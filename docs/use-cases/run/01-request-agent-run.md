...
| UC-RUN-01-S05 | P1 | Follow-up run prompts include only effective issue activity since the previous handled cursor as the primary input, while `.grovie/task.json` keeps the complete structured context as an optional fallback when the prompt is insufficient. |
