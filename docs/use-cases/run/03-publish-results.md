...
| UC-RUN-03-S06 | P0 | A comment-only run writes `.grovie/issue-comment.md`, and Grovie publishes that body as an issue comment without requiring runtime GitHub auth. |
...
| UC-RUN-03-S08 | P0 | A run writes `.grovie/result.json` with action `comment`, and Grovie publishes `comment.body` or the compatible `.grovie/issue-comment.md` body as an issue comment. |
