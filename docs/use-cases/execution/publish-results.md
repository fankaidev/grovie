...
| UC-RESULT-001-S02 | P0 | A coder run with worktree changes commits them, pushes the agent's session branch, opens a PR, and links it from the issue. |
...
| UC-RESULT-001-S05 | P1 | A reviewer run with no intended code changes can post review output without modifying the coder branch. |
