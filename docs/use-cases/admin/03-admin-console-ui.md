...
|----|----------|----------|
| UC-ADMIN-03-S01 | P2 | The React `GET /` route fetches admin APIs and shows daemon health, runtime availability, configured agent availability, useful paths, watched repositories, and recent sessions. |
...
| UC-ADMIN-03-S06 | P1 | The React `GET /` route shows watched repositories, recent sessions, and recent activity as full-width sections from top to bottom, with sessions grouped by issue and agent. |
