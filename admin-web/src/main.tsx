...
function RunDetailPage(props: { runId: string }): ReactNode {
  ...
  async function cancelRun(): Promise<void> {
    ...
    const response = await fetch(`/api/runs/${encodeURIComponent(state.run.runId)}/cancel`, {
      method: "POST",
    });
  ...
  return (
    <main className="page-shell run-page">
        ...
        <InfoPanel title="Actions">
          {canCancel ? (
            <div className="action-stack">
              <button className="danger-button" type="button" onClick={() => void cancelRun()} disabled={cancelState.status === "submitting"}>
                {cancelState.status === "submitting" ? "Canceling..." : "Cancel run"}
              </button>
              <p>{cancelState.message ?? "Active local runs can be canceled from this machine."}</p>
            </div>
          ) : (
      ...
      <section className="logs-grid">
        <LogPanel runId={run.runId} log={state.stdout} />
        <LogPanel runId={run.runId} log={state.stderr} />
      </section>
...
async function loadRunDetail(runId: string): Promise<RunDetailState> {
  const encodedRunId = encodeURIComponent(runId);
  const runResponse = await fetch(`/api/runs/${encodedRunId}`);
  ...
  const [eventsPayload, stdoutPayload, stderrPayload] = await Promise.all([
    fetchJson<{ events: RunEvent[] }>(`/api/runs/${encodedRunId}/events`),
    fetchJson<RunLog>(`/api/runs/${encodedRunId}/logs/stdout`),
    fetchJson<RunLog>(`/api/runs/${encodedRunId}/logs/stderr`),
  ]);
...
function readRoute(pathname: string): { name: "home" } | { name: "run-detail"; runId: string; } {
...
function isCancelableRun(status: RunStatus): boolean {
