...
describe("local state cleanup", () => {
  it("[UC-RUN-04-S08] renders a dry-run cleanup plan without deleting artifacts", () => {
  ...
  it("[UC-RUN-04-S07] removes succeeded session worktrees while preserving run history", () => {
    const paths = createPaths();
    const worktreePath = join(paths.worktreesDir, "session-1");
    writeRun(paths, "run-1", {
      status: "succeeded",
  ...
  it("[UC-RUN-04-S09] skips failed, canceled, active, and stale session worktrees by default", () => {
    const paths = createPaths();
  ...
  it("[UC-RUN-04-S09] refuses to remove metadata worktree paths outside Grovie worktrees", () => {
    const paths = createPaths();
  ...
  it("[UC-RUN-04-S10] removes terminal run directories only when logs are explicitly included", () => {
    const paths = createPaths();
  ...
  it("[UC-RUN-04-S08] parses cleanup retention windows", () => {
