...
describe("agent assignments", () => {
  it("[UC-WORKER-03-S01] builds assignment labels from normalized agent ids", () => {
    expect(buildAgentLabel("coder@fankai-mac")).toBe("agent:coder@fankai-mac");
  });

  it("[UC-WORKER-03-S01] rejects non-normalized agent ids", () => {
  ...
  it("[UC-WORKER-03-S03] returns multiple assigned agent ids", () => {
  ...
  it("[UC-WORKER-03-S05] detects assignments for other machines", () => {
