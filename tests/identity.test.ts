...
describe("identity resolution", () => {
  it("[UC-WORKER-01-S01] resolves hostnames into machine ids", () => {
    expect(resolveMachineId("Fankai MacBook Pro.local")).toBe("fankai-macbook-pro-local");
  });

  it("[UC-WORKER-01-S02] builds full agent ids from agent names and machine ids", () => {
    expect(buildAgentId("Code Reviewer", "fankai-mac")).toBe("code-reviewer@fankai-mac");
  });

  it("[UC-WORKER-01-S03] collapses invalid slug characters and trims leading or trailing dashes", () => {
